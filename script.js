...
document.addEventListener("keydown", (e) => { if (e.key === "w") leftPaddle.dy = -paddleSpeed;
    ...
    function update() { 
        ...
        if ( 
            (ball.x <= leftPaddle.x + paddleWidth && ball.y > leftPaddle.y && ball.y < leftPaddle.y + paddleHeight) || (ball.x >= rightPaddle.x - ball.size && ball.y > rightPaddle.y && ball.y < rightPaddle.y + paddleHeight) ) 
            { 
                ball.vx *= -1; 
            } 
            if (ball.x <= 0) { 
                scoreRight++; resetBall(); 
